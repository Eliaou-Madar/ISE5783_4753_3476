...
public class RayTracerBasic extends RayTracerBase {
     ...
     */
    private Color calcColor(GeoPoint closestPoint, Ray ray, boolean isASS) {
        return calcColor(closestPoint, ray, MAX_CALC_COLOR_LEVEL, INITIAL_K, isASS)
     ...
     */
    private Color calcColor(GeoPoint geoPoint, Ray ray, int level, Double3 k, boolean isASS) {
        Color color = calcLocalEffects(geoPoint, ray, k, isASS);
        return 1 == level ? color : color.add(calcGlobalEffects(geoPoint, ray, level, k, isASS));
    }
     ...
     */
    private Color calcGlobalEffects(GeoPoint intersection, Ray ray, int level, Double3 k, boolean isASS) {
        ...
        return calcGlobalEffect(refractedRay, level, k, material.kT, isASS)
                .add(calcGlobalEffect(reflectedRay, level, k, material.kR, isASS));
    }
     ...
     */
    private Color calcGlobalEffect(Ray ray, int level, Double3 k, Double3 kx, boolean isASS) {
        Double3 kkx = k.product(kx);
        if (kkx.lowerThan(MIN_CALC_COLOR_K)) return Color.BLACK;
        GeoPoint reflectedPoint = findClosestIntersection(ray);
        return reflectedPoint == null ? scene.background : calcColor(reflectedPoint, ray, level - 1, kkx, isASS).scale(kx);
    }
     ...
     */
    private Color calcLocalEffects(GeoPoint geoPoint, Ray ray, Double3 k, boolean isASS) {
     ...
     * @return the color from the lights at the point
     */
    private Color helpSuperSampling(Vector[][] lss, Color lu, Color ld, Color ru, Color rd, int x, int y, int z, int w, GeoPoint geoPoint, Ray ray, Double3 k, LightSource lightSource) {
    ...
    @Override
    public Color traceRay(Ray ray, boolean isASS) {
        GeoPoint closestPoint = findClosestIntersection(ray);
        return closestPoint == null ? scene.background : calcColor(closestPoint, ray, isASS);
    }
}